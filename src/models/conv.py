...
from src.models.common import normalize_imagenet
from src.models import resnet_attention
...
class Resnet_Attention(nn.Module):
    ...
    r""" ResNet_Attention encoder network for image input.
    Args:
        c_dim (int): output dimension of the latent embedding
        normalize (bool): whether the input images should be normalized
        use_linear (bool): whether a final linear layer should be used
    ...
    def __init__(self, c_dim, normalize=True, use_linear=True):
        ...
        self.normalize = normalize
        self.use_linear = use_linear
        self.features = resnet_attention.resnet_attention(pretrained=True)
        self.features.fc = nn.Sequential()
        if use_linear:
            self.fc = nn.Linear(512, c_dim)
        elif c_dim == 512:
...
class Vgg16(nn.Module):
    r""" Vgg16 encoder network for image input.
    Args:
        c_dim (int): output dimension of the latent embedding
        normalize (bool): whether the input images should be normalized
        use_linear (bool): whether a final linear layer should be used
    ...
    def __init__(self, c_dim, normalize=True, use_linear=True):
        ...
        self.normalize = normalize
        self.use_linear = use_linear
        self.features = vgg.vgg16_bn(pretrained=True, num_classes=256)
        self.features.fc = nn.Sequential()
        if use_linear:
            self.fc = nn.Linear(512, c_dim)
        elif c_dim == 512:
