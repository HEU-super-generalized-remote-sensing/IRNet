...
from src.models import encoder_latent, decoder, conv
...
decoder_dict = {
    ...
    'cbatchnorm-short': decoder.DecoderCBatchNorm_Short,
    'SDF': decoder.SDF
}
...
encoder_dict = {
    ...
    'resnet_attention': conv.Resnet_Attention,
    'vgg16': conv.Vgg16,
}
...
class OccupancyNetwork(nn.Module):
    ...
    def compute_sdf_elbo(self, p, occ, inputs, **kwargs):
        """ Computes the expectation lower bound.
        Args:
            p (tensor): sampled points
            occ (tensor): occupancy values for p
        ...
        q_z = self.infer_z(p, occ, c, **kwargs)
        z = q_z.rsample()
        pred_sdf = self.decode(p, z, c, **kwargs).logits

        elbo = torch.mean(torch.abs(occ - pred_sdf / 10))
        # kl = dist.kl_divergence(q_z, self.p0_z).sum(dim=-1)
        t = torch.tensor(0.01).to(self._device)
        kl = torch.mean(torch.ge(torch.abs(pred_sdf / 10 - occ), t).float()) * 100
        rec_error = elbo + kl
