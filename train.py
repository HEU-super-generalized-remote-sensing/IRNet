...
import yaml
import matplotlib
from src import dataloader, data, network
...
parser.add_argument('--exit-after', type=int, default=10000,
...
# Dataset
train_dataset = dataloader.get_dataset('train', cfg)
val_dataset = dataloader.get_dataset('val', cfg)
...
# Model
model = network.get_model(cfg, device=device, dataset=train_dataset)

# Intialize training
npoints = 1000
optimizer = optim.Adam(model.parameters(), lr=1e-4)
# optimizer = optim.SGD(model.parameters(), lr=1e-4, momentum=0.9)
trainer = network.get_trainer(model, optimizer, cfg, device=device)
...
while True:
    ...
    for batch in train_loader:
        ...
        if validate_every > 0 and (it % validate_every) == 0:
            eval_dict = trainer.evaluate_sdf(val_loader)
